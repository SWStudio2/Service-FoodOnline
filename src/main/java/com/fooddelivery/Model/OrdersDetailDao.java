...
public interface OrdersDetailDao extends CrudRepository<OrderDetail, Long> {
	...
	@Query(value="select * from order_detail where order_id = :order_id and MER_ID = :mer_id" , nativeQuery = true)
	public List<OrderDetail> findByOrderIdAndMerId2(@Param("order_id") Long order_id , @Param("mer_id") Long mer_id);
}
